...
║  • Full 3D-printable model (0.45 mm clearance, split shells, all hardware)   ║
║                                                                              ║
...
║   • Log writer buffered (no file-open per message)                a           ║
...
# ─── Top-level user flags ────────────────────────────────────────────────────
# Set to one of: "ALL" | "rom" | "head" | "wave" | "breathing" | "walk" |
#                "run" | "combat" | "transform" | "stability" | "servo"
TARGET_MODULE = "ALL"

# Set True to batch-export every solid body as a high-res STL after simulation.
EXPORT_STL = False
...
def run(context):
    ...
    _log_buffer = []   # FIX: batch-write log lines instead of opening file per call
    ...
    log_msg("=" * 60)
    log_msg("EXECUTION START  v7.0  — Optimus Prime G1")
    log_msg("=" * 60)
    ...
    try:
        ...
        # Force Z-Up orientation so the robot stands upright on the grid
        try:
        ...
        # ═══════════════════════════════════════════════════════════════════
        # 0.045 cm = 0.45 mm — FDM print-in-place clearance
        CLEARANCE  = 0.045
        ...
        # FIX: use correct splitBodyFeatures.createInput / add API
        def split_halves(comp, body, axis='y', offset=0.0):
            try:
                ...
                # Collect the body in an ObjectCollection
                bodies_col = adsk.core.ObjectCollection.create()
                bodies_col.add(body)

                split_input = comp.features.splitBodyFeatures.createInput(
                    body, sp, True)
        ...
        def revolute_joint(name, occ1, occ2, cx, cy, cz, axis_str):
            ...
            try:
                ...
                log_msg(f"Failed revolute joint {name}: {traceback.format_exc()}")
        ...
        def ball_joint(name, occ1, occ2, cx, cy, cz):
            ...
            try:
                geom = _make_joint_geometry(cx, cy, cz)
                ji   = root.asBuiltJoints.createInput(occ1, occ2, geom)
                # FIX: use model-axis-aligned directions that match Z-Up orientation
                ...
                log_msg(f"Failed ball joint {name}: {traceback.format_exc()}")
        ...
        # FIX: CLEARANCE now applied to tt_wheel cut-cavity dimensions
        def tt_wheel(comp, tag, cx, cy, cz, side=1):
            ...
            # FIX: drive-shaft slot also gets clearance
            cut_cavity(comp, box(comp, f"{tag}_CDS", cx+side*3.25, cy, cz,
                                 2.7+cl, 0.54+cl, 0.36+cl))
        ...
        # ═══════════════════════════════════════════════════════════════════
        SPLIT_KEYS = {"Shell", "Link", "Main", "Armor", "Core",
                      "Pod", "Palm", "Block", "Sole"}
        ...
        def build_kinematics():
            ...
            # FIX: Pelvis is the true kinematic root (grounded), not Torso
            if p:
                p.isGrounded = True
        ...
        def validate_kinematics():
            ...
            # FIX: Pelvis is the true ground — it doesn't need a parent joint either
            for comp in comps_list:
                if comp.name not in ("OP_Torso", "OP_Pelvis") and \
                   comp.name not in jointed_comps:
                    orphans.append(comp.name)

            if orphans:
                log_msg("!!! WARNING: ORPHANED COMPONENTS DETECTED !!!")
                for o in orphans:
                    log_msg(f"   -> '{o}' has NO joints — it will not move with the robot.")
            else:
                log_msg("All components successfully bound to kinematic chain. [ OK ]")
        ...
        class SimulationEngine:
            ...
            def __init__(self, root, comps_list, design, app, ui):
                ...
                self._report      = []
                self._cols        = []
                self._strict_mode = False  # FIX: initialise flag used in move_group
            ...
            def _gj(self, name):
                ...
                try:
                    ...
                    if not self._logged_joints:
                        ...
                        log_msg(f"Joint '{name}' not found. Available: {names}")
                    return None
                except Exception:
            ...
            def _refresh(self):
                """Pump events, refresh viewport, honour abort flag."""
            ...
            def move_group(self, targets, steps=20, ease=True):
                ...
                for i in range(1, steps + 1):
                    ...
                    if self._strict_mode:
                        ...
                        if mid_hits:
                            log_msg("!!! FATAL KINEMATIC ERROR — mid-animation collision !!!")
                            log_msg(f"  At {t*100:.1f}% of movement step")
                            for hit in mid_hits:
                                log_msg(f"  -> {hit}")
                            raise Exception("Structural Interpenetration Detected")
            ...
            # ─── POSE LIBRARY ─────────────────────────────────────────────
            # FIX: all poses call reset_all() first to prevent accumulation errors
            ...
            def simulate_walking_advanced(self, num_cycles=4):
                ...
                LIFT   = -26; FWD  = 22; PUSH  = 16
                SWAY   =   9; A_SW = 28; K_FLEX = 20
                W_ROT  =   6; H_BOB =  3; FRAMES = 18
            ...
            def simulate_transformation_to_truck(self):
                """
                9-stage transformation.  Each stage is self-contained and
                does NOT depend on a previous move_group call's end state —
                move_group always reads the current joint value before driving.
                """
            ...
            # ─── MODULE 8c — TRANSFORMATION  Truck → Robot ────────────────
            # FIX: reverse sequence exactly un-does each stage in correct order
            def simulate_transformation_to_robot(self):
                log_msg("MODULE 8c: TRUCK → ROBOT")

                # Reverse Stage 9 — release arm lock
                self.move_group([
                    ("L_Shoulder_Cluster",  0, 'yaw'),
                    ("R_Shoulder_Cluster",  0, 'yaw'),
                ], steps=12)

                # Reverse Stage 8 — un-tuck ankles
                self.move_group([
                    ("L_Ankle_Cluster", 0, 'pitch'),
                    ("R_Ankle_Cluster", 0, 'pitch'),
                    ("L_Ankle_Cluster", 0, 'roll'),
                    ("R_Ankle_Cluster", 0, 'roll'),
                ], steps=14)

                # Reverse Stage 7 — straighten knees
                self.move_group([
                    ("L_Knee", 0, 'pitch'),
                    ("R_Knee", 0, 'pitch'),
                ], steps=18)

                # Reverse Stage 6 — extend hips
                self.move_group([
                    ("L_Hip_Cluster", 0, 'pitch'), ("R_Hip_Cluster", 0, 'pitch'),
                    ("L_Hip_Cluster", 0, 'yaw'),   ("R_Hip_Cluster", 0, 'yaw'),
                ], steps=20)

                # Reverse Stage 5 — fold waist back up
                self.move_group([
                    ("Waist_Cluster", 0, 'pitch'),
                ], steps=22)

                # Reverse Stage 4 — raise head
                self.move_group([
                    ("Neck_Cluster", 0, 'pitch'),
                    ("Neck_Cluster", 0, 'yaw'),
                ], steps=12)

                # Reverse Stage 3 — shoulder pitch + sweep + roll to neutral
                self.move_group([
                    ("L_Shoulder_Cluster", 0, 'pitch'), ("R_Shoulder_Cluster", 0, 'pitch'),
                    ("L_Shoulder_Cluster", 0, 'yaw'),   ("R_Shoulder_Cluster", 0, 'yaw'),
                    ("L_Shoulder_Cluster", 0, 'roll'),  ("R_Shoulder_Cluster", 0, 'roll'),
                ], steps=20)

                # Reverse Stage 2 — unfold elbows
                self.move_group([
                    ("L_Elbow", 0, 'pitch'),
                    ("R_Elbow", 0, 'pitch'),
                ], steps=15)

                # Reverse Stage 1 — un-stow wrists
                self.move_group([
                    ("R_Wrist", 0, 'pitch'),
                    ("L_Wrist", 0, 'pitch'),
                ], steps=12)
                ...
                try:
                    ...
                    if res and res.count > 0:
                        for i in range(res.count):
                            r = res.item(i)
                            if not (r.interferenceBody and
                                    r.interferenceBody.volume > vol_min):
                                continue
                            comp1 = (r.entityOne.parentComponent.name
                                     if r.entityOne.parentComponent else "?")
                            comp2 = (r.entityTwo.parentComponent.name
                            ...
                            try:
                                ...
                                loc_str = "XYZ:(Unknown)"
                            hit_str = (f"[{comp1}/{r.entityOne.name}] ↔ "
                                       f"[{comp2}/{r.entityTwo.name}] | "
                                       f"Vol:{r.interferenceBody.volume:.3f} cm³ | {loc_str}")
                            hits.append(hit_str)
                            log_msg(f"COLLISION: {hit_str}")
                    return list(set(hits))
                except Exception:
                    log_msg(f"_check error: {traceback.format_exc()}")
                    return []
            ...
            def export_urdf(self, path):
                try:
                    ...
                    L = ['<?xml version="1.0"?>', '<robot name="optimus_prime_g1">']
                    for comp in self._comps:
                    ...
                    # FIX: use indexed iteration — asBuiltJoints is not directly iterable
                    for i in range(self._root.asBuiltJoints.count):
                        ...
                        if mo.objectType == adsk.fusion.RevoluteJointMotion.classType():
                            jt = "revolute"
                        elif mo.objectType == adsk.fusion.RigidJointMotion.classType():
                            jt = "fixed"
                        else:
                            jt = "floating"
            ...
            def run_all_simulations(self):
                ...
                SEP  = "═" * 54
                msg  = f"{SEP}\n"
                msg += "   OPTIMUS PRIME G1 — FULL SIMULATION REPORT  v7.0\n"
                msg += f"{SEP}\n\n"
                msg += "\n".join(self._report)
                msg += f"\n\n{SEP}\n  COLLISION SUMMARY\n{SEP}\n"
                if unique_cols:
                ...
                msg += f"Log   : C:\\opt_fusion_log.txt\n"
                msg += f"{SEP}\n"
        ...
        log_msg(f"--- BEGINNING SIMULATION [TARGET: {TARGET_MODULE}] ---")

        # FIX: all method names corrected to match actual SimulationEngine methods
        dispatch = {
            "ALL":        sim.run_all_simulations,
            "rom":        sim.test_joint_rom,
            "head":       sim.simulate_head_look,
            "wave":       sim.simulate_wave,
            "breathing":  lambda: sim.simulate_idle_breathing(cycles=4),
            "walk":       lambda: sim.simulate_walking_advanced(num_cycles=4),
            "run":        lambda: sim.simulate_running(num_cycles=3),
            "combat":     sim.simulate_combat_sequence,
            "transform":  lambda: (sim.simulate_transformation_to_truck(),
                                   sim.simulate_truck_driving(),
                                   sim.simulate_transformation_to_robot()),
            "stability":  sim.run_stability_analysis,
            "servo":      sim.estimate_servo_loads,
        }

        fn = dispatch.get(TARGET_MODULE)
        if fn:
            fn()
        else:
            log_msg(f"Unknown TARGET_MODULE '{TARGET_MODULE}', running ALL.")
            sim.run_all_simulations()

        # ── STL BATCH EXPORT ───────────────────────────────────────────────
        if EXPORT_STL:
            log_msg(f"STL export → {EXPORT_DIR}")
                ...
                for body in comp.bRepBodies:
                    if body.isSolid and not any(t in body.name for t in SKIP_EXPORT):
                        ...
                        try:
                            ...
                            log_msg(f"STL export failed: {comp.name}/{body.name}")
        ...
        if "SIMULATION_ABORTED_BY_USER" in str(e):
            msg = "ABORTED: Simulation terminated by external stop flag."
            log_msg(msg)
            if ui:
