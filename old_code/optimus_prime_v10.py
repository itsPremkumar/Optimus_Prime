# ─────────────────────────────────────────────────────────────────────────────
# OPTIMUS PRIME G1  v10.0  ─ Physical Build Edition
...
LOG_FILE       = os.path.join(LOG_DIR,     f"optimus_v10_{_ts}.txt")
BOM_FILE       = os.path.join(_OUTPUT_DIR, f"BOM_v10_{_ts}.csv")
...
def run(context):
    ...
    Logger.log("=" * 60)
    Logger.log("EXECUTION START  v10.0 -- Optimus Prime G1 Physical Build")
    Logger.log("=" * 60)
    ...
    try:
        ...
        def bearing(comp, tag, cx, cy, cz, axis="x", ro=1.10, w=0.60):
            """Centred bearing cavity — FIX from v9 maintained."""
        ...
        class SimulationEngine:
            ...
            def export_urdf(self):
                ...
                try:
                    ...
                    path = os.path.join(EXPORT_DIR, "robot_v10.urdf")
            ...
            # ── SIM-4: BOM Export ─────────────────────────────────────────
            def export_bom(self):
                """SIM-4 — Finalise and write BOM CSV. Also write wiring map."""
            ...
            def run_all_simulations(self):
                ...
                # ── Final Report ──────────────────────────────────────────
                Logger.log("=" * 55)
                Logger.log("OPTIMUS PRIME G1 v10.0 — FINAL REPORT")
                Logger.log("=" * 55)
                ...
                if EXPORT_URDF:
                    Logger.log(f"  URDF  → {EXPORT_DIR}/robot_v10.urdf")
        ...
        try:
            os.makedirs(EXPORT_DIR, exist_ok=True)
            archive_path = os.path.join(EXPORT_DIR, "Optimus_Prime_G1_v10.f3d")
        ...
        Logger.log("v10 script finished successfully.")
