# ─────────────────────────────────────────────────────────────────────────────
# OPTIMUS PRIME G1  v13.0  ─ Jetson Nano AI Brain Edition
# Fusion 360 Python Script  |  Production-Ready Physically Buildable Robot
# ─────────────────────────────────────────────────────────────────────────────
# V13 ARCHITECTURE: NVIDIA Jetson Nano replaces RPi Zero as robot brain
#
# SYSTEM OVERVIEW
# ───────────────
# [BRAIN]  NVIDIA Jetson Nano (compact carrier, 7.0×4.8 cm)
#            - Python/ROS2, OpenCV, TensorRT, high-level AI
#            - CSI-2 camera port → IMX219 camera (robot eyes)
#            - USB3 ×2, USB2 ×2, 40-pin GPIO
#
# [CTRL1]  ESP32-S3 Node A (lower body: hips, knees, ankles)
#            - FreeRTOS, real-time servo interpolation
#            - I2C → PCA9685 #1 (hips/knees), PCA9685 #2 (ankles)
#            - I2C → MPU-6050 IMU (pelvis balance)
#
# [CTRL2]  ESP32-S3 Node B (upper body: waist, shoulders, elbows)
#            - I2C → PCA9685 #3 (shoulders/elbows/waist)
#            - I2C → INA3221 current sensor (servo rail health)
#
# [CTRL3]  ESP32-S3 Node C (head + hands)
#            - I2C → PCA9685 #4 (neck/wrist/fingers)
#            - I2C → VL53L1X ToF ×2 (front obstacle avoidance)
#            - UART → Jetson debug console
#
# [COMMS]  Jetson ↔ each ESP32-S3 via USB-C ↔ CH340 serial bridge
#            - 1 Mbaud UART, COBS framing, CRC-16 error detection
#            - Fallback: WiFi WebSocket (ESP32 AP mode)
#
# [POWER]  2S 7.4V LiPo → 5V/4A UBEC ×2 (Jetson + logic)
#                        → 7.4V servo rail (through 5A ATO fuse)
#
# V13 CHANGES vs V12
# ──────────────────
# ELEC-V13-1  jetson_nano_bay()     - Jetson Nano compact carrier pocket
#                                     + 30mm fan mount + heatsink space
#                                     + connector access (USB, CSI, HDMI, GPIO)
# ELEC-V13-2  csi_camera_pocket()   - IMX219 Jetson CSI-2 camera in head
#                                     (replaces ESP32-CAM)
# ELEC-V13-3  fpc_ribbon_channel()  - FPC ribbon routing head→neck→torso
# ELEC-V13-4  esp32s3_node_bay()    - ESP32-S3 control node pocket (×3)
# ELEC-V13-5  tof_sensor_pocket()   - VL53L1X ToF obstacle sensor
# ELEC-V13-6  ina3221_bay()         - 3-channel current/power monitor
# ELEC-V13-7  ubec_5v4a_bay()       - 5V/4A power regulator for Jetson
# ELEC-V13-8  vent_grille()         - Jetson thermal management vents
# ELEC-V13-9  antenna_clearance()   - ESP32 WiFi/BT antenna keep-out zone
# ELEC-V13-10 uart_bridge_cutout()  - USB-C serial bridge connector access
#
# ARCH-V13-1  log_v13_architecture() - Log full system topology
# ARCH-V13-2  log_comms_map()        - Log UART/I2C/GPIO bus assignments
# ARCH-V13-3  log_power_budget()     - Log power rail estimates
# ARCH-V13-4  log_ai_pipeline()      - Log vision/AI processing chain
#
# SIM-V13-1  validate_v13_architecture() - Verify all components placed
# SIM-V13-2  simulate_vision_tracking()  - Head tracking camera coverage
# SIM-V13-3  simulate_obstacle_reaction()- ToF-based avoidance sequence
#
# BUGFIX-V13-1  move_ball(): fixed `ax` name collision (was using undefined
#               variable `ax` instead of loop variable `axis` in _set calls)
# BUGFIX-V13-2  merge_fasteners_to_halves(): added None guards on body args
# BUGFIX-V13-3  verify_screw_lengths(): guard on empty SCREW_REGISTRY
# BUGFIX-V13-4  export_step(): robust skip-tag check
# BUGFIX-V13-5  capture_screenshots(): graceful fallback when turntable fails
# ─────────────────────────────────────────────────────────────────────────────
...
BOM_FILE       = os.path.join(_OUT,     f"BOM_v13_{_ts}.csv")
ASSEMBLY_FILE  = os.path.join(_OUT,     f"ASSEMBLY_GUIDE_v13_{_ts}.txt")
MANIFEST_FILE  = os.path.join(_OUT,     f"BUILD_MANIFEST_v13_{_ts}.json")
PRODUCTION_FILE= os.path.join(_OUT,     f"PRODUCTION_v13_{_ts}.txt")
...
def run(context):
    ...
    Logger.log("=" * 64)
    Logger.log("EXECUTION START  v13.0 -- Optimus Prime G1 Jetson AI Brain Edition")
    ...
    try:
        ...
        def write_assembly_guide():
            """DOC-1 — Write ASSEMBLY_GUIDE.txt (v13: Jetson/ESP32-S3 wiring,
        ...
        def write_build_manifest():
            """Write a machine-readable manifest including v13 comm/power maps."""
            try:
        ...
        def write_production_readiness_report(check_rows=None):
            """Write the final v13 production-readiness checklist."""
        ...
        class SimulationEngine:
            """
            Simulation suite for Optimus Prime G1 v13 (Jetson Nano AI Brain
            Edition). All v12 mechanical/physical validation features are
            retained; new in v13: architecture validation, vision-tracking
            ...
            def validate_v13_architecture(self):
                """SIM-V13-1 — Verify all v13 electronics components were
            ...
            def run_v13_validation(self):
                ...
                self.validate_v13_architecture()
