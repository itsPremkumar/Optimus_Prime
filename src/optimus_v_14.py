...
# ── V14 NEW (renamed by AXIS-1 fix): knee-ROLL servo/bearing geometry ─────────
...
# Every "*_Cluster"/ball-joint entry in JOINT_LIMITS is simulated as a full
# 3-DOF Fusion ball joint regardless of how many physical servos were
# actually modeled for it. That decoupling is exactly what let several
# joints ship with one axis doubled up on hardware (see OP_UpperArm_*
# shoulder roll below) and another axis with no actuator at all (waist
# roll, neck roll, ankle yaw, one of the two wrist DOF). Every servo call
# for a multi-axis joint now also registers here so
# verify_joint_axis_mapping() can catch that class of bug automatically
# instead of it only showing up as "the shoulder twists the wrong way."
...
def run(context):
    ...
    try:
        ...
        # ── AXIS-1 BUGFIX: automated joint axis-mapping verification ─────
        @trace_execution
        def verify_joint_axis_mapping():
            """AXIS-1 -- For every joint in JOINT_LIMITS with 2+ rotational
            DOF, confirm every declared DOF (pitch/yaw/roll) has a real
            servo+bearing registered against a physical axis, and that no
            two DOF on the same joint were ever registered against the same
            axis. This is the automated version of the bug this file used to
            have: two servos silently sharing one axis while a third axis
            had no actuator at all."""
            Logger.log("--- V14 JOINT AXIS-MAPPING VERIFICATION (AXIS-1) ---")
            # Thumb CMC is a compliant, tendon+spring driven joint (see
            # servo_drum / tendon_anchor in the hand/finger section) -- it
            # has no direct per-axis servo by design, so it's excluded here
            # rather than flagged as "missing" on every single run.
            ...
            for joint, dof_limits in JOINT_LIMITS.items():
                ...
                if len(rot_dofs) < 2:
                    continue  # single-DOF hinges can't collide with themselves
                checked += 1
        ...
        # AXIS-1 FIX: Waist_Cluster declares pitch+yaw+roll in JOINT_LIMITS but
        # V14 only ever built the first two servos -- roll had no actuator at
        # all, even though the ball joint happily allows it in simulation.
        ...
        # AXIS-1 FIX: Neck_Cluster declares roll in JOINT_LIMITS but V14 never
        # built a roll servo -- add one on Y so all 3 declared DOF exist and
        # each sits on its own axis (pitch=X in torso, yaw=Z above, roll=Y here).
        ...
        for side, sx in [("L", -HIP_X), ("R", HIP_X)]:
            ...
            # AXIS-1 FIX (was "MECH-V14-1: knee-yaw"): this DOF lets the lower
            # leg splay sideways for uneven-terrain adaptation. Splaying a
            # vertically-hanging shin is a rotation about the Y axis (this
            # file's ROLL convention -- see hip/shoulder roll, also Y), NOT
            # about Z ("yaw" everywhere else). The servo/bearing below were
            # already correctly built on "y"; only the name and the
            # JOINT_LIMITS key were wrong, which made the simulation drive a
            # phantom Z-axis "yaw" that no bearing in this design can
            # actually produce, while the real Y-axis hardware was never
            # commanded by name anywhere. Renamed Yaw->Roll throughout.
            # Mounted offset from the pitch axis so the two servos don't
            # physically clash.
            ...
            # AXIS-1 FIX: AnkR (roll) never had its own bearing -- add one.
            ...
            # AXIS-1 FIX: Ankle_Cluster declares a (-30,95) deg yaw range in
            # JOINT_LIMITS but V14 never built a yaw servo or bearing for it --
            # the range existed only because the generic ball joint grants it
            # for free. Added the missing actuator on Z.
            mg996r(foot, f"{side}_AnkY", sx, 0, ANKLE_CTR+1.4, "z")
            bearing_fit(foot, f"{side}_AnkY_Brg", sx, 0, ANKLE_CTR+1.4, "z", 0.90, 0.50, fit_type="press")
        ...
        for side, ax in [("L", -SHOULDER_X), ("R", SHOULDER_X)]:
            ...
            # AXIS-1 FIX: this used to be bearing_fit(...,"{side}_SB",..., "x", ...)
            # -- an exact duplicate of {side}_ShP_Dual's axis AND location, which
            # gave the PITCH axis a second bearing while ShR (roll, Y axis) had
            # none at all. Re-pointed to the roll servo's own location/axis.
            ...
            # AXIS-1 FIX: this servo was named "{side}_WR" (Wrist Roll) but was
            # physically built on the X axis, which is this file's PITCH
            # convention everywhere else -- and it was the ONLY wrist servo,
            # even though L/R_Wrist declares both pitch and roll in
            # JOINT_LIMITS. Renamed to reflect what it actually drives, and
            # added a real roll servo on Y so pitch and roll each have their
            # own distinct axis and their own bearing.
        ...
        # ── AXIS-1: Joint axis-mapping verification (bugfix) ──────────────
        ...
        class SimulationEngine:
            ...
            def simulate_walking_advanced(self):
                ...
                for cycle in range(4):
                    ...
                    self.move_ball([
                        ...
                        # AXIS-1 FIX: L/R_Knee's second DOF is "roll" (Y axis,
                        # matching the KneRoll servo), not "yaw" -- moved from
                        # the 3rd tuple slot (yaw) to the 4th (roll).
            ...
            def simulate_running(self):
                ...
                for cycle in range(3):
                    ...
                    self.move_ball([
                        ...
                        # AXIS-1 FIX: see simulate_walking_advanced -- roll, not yaw.
            ...
            def estimate_servo_loads(self):
                ...
                loads = [
                    ...
                    # AXIS-1 FIX: load rows for the servos added/renamed to
                    # close out each joint's 3rd (or 2nd) axis. Figures are
                    # rough estimates in the same spirit as the rows above --
                    # re-check against real hardware before final servo choice.
