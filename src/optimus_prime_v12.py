# ─────────────────────────────────────────────────────────────────────────────
# OPTIMUS PRIME G1  v12.0  ── Physically Buildable Prototype Edition
# Fusion 360 Python Script  |  Anthropic MCP-compatible
# ─────────────────────────────────────────────────────────────────────────────
# WHAT'S NEW IN v12  (Physical Engineering Upgrade from v10)
# ──────────────────────────────────────────────
...
ASSEMBLY_FILE  = os.path.join(_OUTPUT_DIR, f"ASSEMBLY_GUIDE_v12_{_ts}.txt")
MANIFEST_FILE  = os.path.join(_OUTPUT_DIR, f"BUILD_MANIFEST_v12_{_ts}.json")
PRODUCTION_FILE = os.path.join(_OUTPUT_DIR, f"PRODUCTION_READINESS_v12_{_ts}.txt")
...
# ── V12 NEW: Tendon / finger drive parameters ────────────────────────────────
...
# ── V12 NEW: Screw length registry for verification ──────────────────────────
...
# ── V12 NEW: Assembly step registry ──────────────────────────────────────────
ASSEMBLY_STEPS = []
JIG_REGISTRY   = []
...
# ═════════════════════════════════════════════════════════════════════════════
# BOM TRACKER  (v12 enhanced with new categories)
# ═════════════════════════════════════════════════════════════════════════════
...
def run(context):
    ...
    Logger.log("=" * 60)
    Logger.log("EXECUTION START  v12.0 -- Optimus Prime G1 Production Build")
    ...
    Logger.log("V12 audit: production manifest, visual sweep, export readiness")
    Logger.log("=" * 60)
    ...
    try:
        ...
        # ── V12 NEW: Printable chamfer helper ─────────────────────────────
        ...
        # ── V12 NEW: Post-split fastener merge ────────────────────────────
        ...
        def m3_boss(comp, tag, cx, cy, cz, axis="z", depth=0.80, screw_len=1.0):
            ...
            # V12: register for screw verification
        ...
        # ── V12 NEW: FST-1  verify_screw_lengths ──────────────────────────
        def verify_screw_lengths():
            ...
            Logger.log("--- V12 SCREW LENGTH VERIFICATION ---")
            issues = 0
        ...
        def write_assembly_guide():
            ...
            try:
                ...
                with open(ASSEMBLY_FILE, "w", encoding="utf-8") as f:
                    f.write("=" * 60 + "\n")
                    f.write("  OPTIMUS PRIME G1 v12.0  ASSEMBLY GUIDE\n")
        ...
        def write_build_manifest():
            """Write a machine-readable manifest for V12 production review."""
            try:
        ...
        def rpi_zero_bay(comp, tag, cx, cy, cz):
            ...
            # V12: connector openings for USB, power, SD card access
        ...
        def pca9685_bay(comp, tag, cx, cy, cz):
            ...
            # V12: JST connector clearance slots (16 channels)
        ...
        def lipo_bay(comp, tag, cx, cy, cz):
            ...
            # V12: battery retention strap slots (2x)
        ...
        def esp32_cam_pocket(comp, tag, cx, cy, cz, lens_axis="y"):
            ...
            # V12: programming header access slot
        ...
        # ─────────────────────────────────────────────────────────────────
        # HARDWARE MODULES  (with V12 coupler integration)
        # ─────────────────────────────────────────────────────────────────
        ...
        def mg996r(comp, tag, cx, cy, cz, axis="x"):
            ...
            # V12: install horn coupler on output shaft
        ...
        def mg90s(comp, tag, cx, cy, cz, axis="x"):
            ...
            # V12: install micro horn coupler
        ...
        def bearing(comp, tag, cx, cy, cz, axis="x", ro=1.10, w=0.60):
            """V12: Legacy bearing call — redirects to bearing_fit with glue-fit default."""
        ...
        # ═════════════════════════════════════════════════════════════════
        # COMPONENT BUILDING  (V12 Physically Buildable Edition)
        # ═════════════════════════════════════════════════════════════════

        # ─────────────────────────────────────────────────────────────────
        # 1 TORSO  (GEO-4, ELEC-1, ELEC-2, ELEC-3  + V12 covers, cable, BEC)
        # ─────────────────────────────────────────────────────────────────
        ...
        # V12 MECH-1 — Internal structural skeleton (upgraded)
        ...
        # V12 CAB-2 — Torso wire hub
        wire_hub(torso, "TorsoHub", 0, 1.5, TORSO_CTR+0.5)

        # V12 CAB-1 — Cable clips along wire runs
        ...
        # V12 POW-4 — BEC mount (near LiPo bay)
        bec_mount(torso, "MainBEC", LIPO_L/2 + 1.0, 3.0, TORSO_CTR-2.0)

        # V12 POW-3 — Power switch (side panel)
        ...
        # V12: Printability check on torso parts
        for b in list(torso.bRepBodies):
            if b.name:
                printability_check(torso, b.name)

        # ─────────────────────────────────────────────────────────────────
        # 2 HEAD  (GEO-1  + V12 ESP32-CAM cover)
        # ─────────────────────────────────────────────────────────────────
        ...
        # GEO-1 — Chin guard (V12: chamfered for printability)
        ...
        # V12 COV-3 — ESP32-CAM cover (hinged for lens access)
        ...
        # ─────────────────────────────────────────────────────────────────
        # 3 PELVIS  (ELEC-6  + V12 IMU cover, cable routing)
        # ─────────────────────────────────────────────────────────────────
        ...
        # ─────────────────────────────────────────────────────────────────
        # 4 LEGS  (GEO-5 improved feet  + V12 tendon-ready hands, couplers)
        # ─────────────────────────────────────────────────────────────────
        for side, sx in [("L", -HIP_X), ("R", HIP_X)]:
            ...
            # V12 MECH-1: Internal rib for torsional rigidity
            ...
            # V12 BRG-3: dual bearing for hip pitch (high load)
            ...
            # V12 MECH-4: knee transform lock
            ...
            # V12 MECH-1: shin reinforcing rib
            ...
            # V12 CAB-3: wire grommet at ankle exit
            grommet_slot(shin, f"{side}_AnkleWire", sx, 0, SHIN_CTR-5.0, "z", 0.50)

            # —— FOOT  (GEO-5 + V12 stability improvements) ——
            ...
            # GEO-5 — Heel spur (V12: chamfered edges for printability)
            ...
            # V12 MFG-2: heel spur printability — add chamfer
            ...
            # Ankle servos (V12: couplers + bearing fit)
        ...
        # ─────────────────────────────────────────────────────────────────
        # 5 ARMS  (GEO-3 shoulders + GEO-2 hands  + V12 tendon drive)
        # ─────────────────────────────────────────────────────────────────
        for side, ax in [("L", -SHOULDER_X), ("R", SHOULDER_X)]:
            ...
            # Twin spike stacks (V12: cone tips chamfered for printability)
            for sz, sr, sh2 in [(SHOULDER_CTR+2.8, 0.42, 0.95),
                ...
                # V12: replace sharp cone with printable chamfered tip
            ...
            # V12 MECH-1: internal shoulder frame
            ...
            # Shoulder servos (3 DOF) — V12: couplers + dual bearings
            mg996r(ua, f"{side}_ShY",    ax, 0, SHOULDER_CTR+1.5, "z")
            # V12 BRG-3: dual bearing for shoulder yaw
            ...
            # Elbow joint — V12: coupler + dual bearing
            ...
            # V12 CAB-3: wire grommet at wrist
            grommet_slot(fa, f"{side}_WristWire", ax, 0, WRIST_Z, "y", 0.45)

            # —— HAND (palm + V12 tendon system) ——
            ...
            # V12: Finger servo bay (DS04-NFC) with drum
            box(hand, "Finger_SrvBay", ax, 0.5, WRIST_Z-1.8, 1.4, 0.90, 2.4, black_plastic)
            # V12 PHY-7: servo drum for tendon winding
            ...
            # V12 PHY-10: palm idler pulleys (route tendon to each finger)
            ...
            # GEO-2 + V12 PHY-8/9/10/11: Articulated fingers with tendon drive
            for fi, (fname, fxo, pp_l, mp_l, dp_l) in enumerate(
                ...
                # V12 PHY-9: tendon anchor at distal phalanx tip
                ...
                # V12 CAB-3: wire grommet (tendon exit) at MCP
                grommet_slot(fc, f"{fname}_TendonExit", fx, fy-0.2, mcp_z, "y", 0.30)

            # GEO-2 + V12: Thumb (2-phalanx, angled outward)
            ...
            # V12: tendon guide + spring on thumb
            ...
            # V12: Printability + jigs for arm components
        ...
        # V12: cable passthrough from backpack to torso
        ...
        # ─────────────────────────────────────────────────────────────────
        # FDM SHELL SPLITTING (Y-plane @ 0)  +  V12 MFG-1 fastener merge
        # ─────────────────────────────────────────────────────────────────
        Logger.log("--- FDM Shell Splitting + Fastener Merge (V12) ---")
        for comp in comps_list:
            to_split = [b for b in comp.bRepBodies
                        if b.name and any(k in b.name for k in SPLIT_KEYS)]
            for b in to_split:
                split_halves(comp, b, "y", 0.0)
            # V12 MFG-1: merge fasteners to nearest half
        ...
        # ── V12 FST-1: Screw length verification ─────────────────────────
        verify_screw_lengths()

        # ── V12 DOC-1: Write assembly guide ──────────────────────────────
        write_assembly_guide()

        # ═════════════════════════════════════════════════════════════════
        # SIMULATION ENGINE  v12.0
        # ═════════════════════════════════════════════════════════════════

        class SimulationEngine:
            """
            Simulation suite for Optimus Prime G1 v12 (Physical Build Edition).
            New in v12: All v10 sim features + tendon drive validation,
            ...
            def estimate_servo_loads(self):
                Logger.log("MODULE 9b: SERVO LOAD ESTIMATION (v12 upgraded servos)")
                loads = [
            ...
            # ── V12 NEW: Module 10: Full physical validation ──────────────
            def run_physical_validation(self):
                """Module 10 — V12 comprehensive build validation."""
                Logger.log("--- MODULE 10 / 10 ---")
                Logger.log("MODULE 10: PHYSICAL BUILD VALIDATION (v12)")
            ...
            def simulate_robot_mode(self):
                ...
                try:
                    ...
                    pass
                self.capture_screenshots("optimus_robot_v12")
            ...
            def simulate_truck_mode(self):
                ...
                try:
                    ...
                    pass
                self.capture_screenshots("optimus_truck_v12")
            ...
            def simulate_battle_mode(self):
                ...
                self.capture_screenshots("optimus_battle_v12")

            def run_visual_audit(self):
                """Capture robot, truck, and battle poses from all review angles."""
            ...
            # ── URDF Export (v12 — proper joint types + inertia) ──────────
            def export_urdf(self):
                ...
                try:
                    ...
                    path = os.path.join(EXPORT_DIR, "robot_v12.urdf")
                    jc   = self._root.asBuiltJoints.count
                    with open(path, "w", encoding="utf-8") as f:
                        f.write('<?xml version="1.0" encoding="utf-8"?>\n')
                        f.write('<robot name="Optimus_Prime_G1_v12">\n\n')
                        for comp in self._comps:
                        ...
                        for i in range(jc):
                            ...
                            f.write( '  </joint>\n')
                        f.write('</robot>\n')
                    Logger.log(f"URDF v12 exported -> {path}")
                except Exception:
            ...
            def export_stl(self):
                try:
                    ...
                    # V12: export jigs to separate folder
            ...
            def export_step(self):
                try:
                    os.makedirs(EXPORT_DIR, exist_ok=True)
                    export_mgr = self._design.exportManager
                    path       = os.path.join(EXPORT_DIR, "Optimus_Prime_G1_v12.step")
            ...
            # ── SIM-4: BOM Export ─────────────────────────────────────────
            def export_bom(self):
                # V12: expanded BOM with all new categories
            ...
            def run_all_simulations(self):
                dispatch = {
                    "ALL": lambda: [
                        ...
                        self.run_physical_validation(),  # V12 module 10
                ...
                # ── Final Report ──────────────────────────────────────────
                Logger.log("=" * 55)
                Logger.log("OPTIMUS PRIME G1 v12.0 — FINAL REPORT")
                ...
                if EXPORT_URDF:
                    Logger.log(f"  URDF  -> {EXPORT_DIR}/robot_v12.urdf")
                ...
                Logger.log("=" * 55)
                Logger.log("V12 BUILD COMPLETE — Review ASSEMBLY_GUIDE before printing")
        ...
        try:
            os.makedirs(EXPORT_DIR, exist_ok=True)
            archive_path = os.path.join(EXPORT_DIR, "Optimus_Prime_G1_v12.f3d")
        ...
        Logger.log("v12 script finished successfully.")
