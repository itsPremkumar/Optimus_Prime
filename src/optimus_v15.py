...
LOG_FILE       = os.path.join(LOG_DIR,  f"optimus_v15_{_ts}.txt")
BOM_FILE       = os.path.join(_OUT,     f"BOM_v15_{_ts}.csv")
ASSEMBLY_FILE  = os.path.join(_OUT,     f"ASSEMBLY_GUIDE_v15_{_ts}.txt")
MANIFEST_FILE  = os.path.join(_OUT,     f"BUILD_MANIFEST_v15_{_ts}.json")
PRODUCTION_FILE= os.path.join(_OUT,     f"PRODUCTION_v15_{_ts}.txt")
...
class DebugManager:
    ...
    def _global_exception_handler(cls, exc_type, exc_value, exc_traceback):
        ...
        try:
            dump_path = os.path.join(_OUT, "CRASH_DUMP_v15.json")
    ...
    def generate_debug_report(cls):
        ...
        report_path = os.path.join(_OUT, f"OPTIMUS_DEBUG_REPORT_v15.txt")
        try:
...
def run(context):
    ...
    Logger.log("=" * 64)
    Logger.log("EXECUTION START  v15.0 -- Optimus Prime G1 Unified Distributed AI Edition")
    Logger.log("Brain: NVIDIA Jetson Nano | Nodes: ESP32-S3 x3 | Eyes: CSI camera")
    Logger.log("V15 bugfixes: split_halves() NameError (fastener merge was silently")
    Logger.log("  a no-op), DOFValidator/CADTracker never wired up, double-injection")
    Logger.log("  of @trace_execution methods, E-stop/status-RGB/sensor-array/")
    ...
    try:
        ...
        # the log output and BOM implied those subsystems existed. v15 now
        ...
        def write_assembly_guide():
            try:
                ...
                with open(ASSEMBLY_FILE, "w", encoding="utf-8") as f:
                    f.write("=" * 64 + "\n")
                    f.write("  OPTIMUS PRIME G1 v15.0  ASSEMBLY GUIDE\n")
        ...
        def write_production_readiness_report(check_rows=None):
            ...
            try:
                ...
                with open(PRODUCTION_FILE, "w", encoding="utf-8") as f:
                    f.write("OPTIMUS PRIME G1 v15.0 PRODUCTION READINESS\n")
                    f.write("Jetson Nano AI Brain Edition\n")
                    f.write("=" * 56 + "\n\n")
        ...
        # ═════════════════════════════════════════════════════════════════
        # COMPONENT BUILDING  (v15 Jetson Nano AI Brain Edition)
        # ═════════════════════════════════════════════════════════════════
        ...
        # ─────────────────────────────────────────────────────────────────
        Logger.log("--- FDM Shell Splitting + Fastener Merge (v15) ---")
        for comp in comps_list:
        ...
        # ═════════════════════════════════════════════════════════════════
        # SIMULATION ENGINE  v15.0
        # ═════════════════════════════════════════════════════════════════

        class SimulationEngine:
            """
            Simulation suite for Optimus Prime G1 v15 (Unified Distributed AI
            ...
            def simulate_robot_mode(self):
                ...
                try:
                    ...
                    pass
                self.capture_screenshots("optimus_robot_v15")
            ...
            def simulate_truck_mode(self):
                ...
                try:
                    ...
                    pass
                self.capture_screenshots("optimus_truck_v15")
            ...
            def simulate_battle_mode(self):
                ...
                self.capture_screenshots("optimus_battle_v15")
            ...
            def export_urdf(self):
                ...
                try:
                    ...
                    path = os.path.join(EXPORT_DIR, "robot_v15.urdf")
                    jc   = self._root.asBuiltJoints.count
                    with open(path, "w", encoding="utf-8") as f:
                        f.write('<?xml version="1.0" encoding="utf-8"?>\n')
                        f.write('<robot name="Optimus_Prime_G1_v15">\n\n')
                        for comp in self._comps:
                        ...
                        for i in range(jc):
                            ...
                            f.write( '  </joint>\n')
                        f.write('</robot>\n')
                    Logger.log(f"URDF v15 exported -> {path}")
                except Exception:
            ...
            def export_step(self):
                try:
                    os.makedirs(EXPORT_DIR, exist_ok=True)
                    export_mgr = self._design.exportManager
                    path = os.path.join(EXPORT_DIR, "Optimus_Prime_G1_v15.step")
            ...
            def run_all_simulations(self):
                ...
                Logger.log("=" * 58)
                Logger.log("OPTIMUS PRIME G1 v15.0 -- FINAL REPORT")
                ...
                if EXPORT_URDF:
                    Logger.log(f"  URDF  -> {EXPORT_DIR}/robot_v15.urdf")
                ...
                Logger.log("=" * 58)
                Logger.log("v15 BUILD COMPLETE -- Review ASSEMBLY_GUIDE before printing")
        ...
        try:
            os.makedirs(EXPORT_DIR, exist_ok=True)
            archive_path = os.path.join(EXPORT_DIR, "Optimus_Prime_G1_v15.f3d")
        ...
        Logger.log("v15 script finished successfully.")
