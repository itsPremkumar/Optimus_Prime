# ─────────────────────────────────────────────────────────────────────────────
# OPTIMUS PRIME G1  v13.0  ── Distributed AI & Real-Time Control Edition
# Fusion 360 Python Script  |  Jetson Nano + ESP32 Node Architecture
# ─────────────────────────────────────────────────────────────────────────────
# WHAT'S NEW IN v13  (Architectural Upgrade from v12)
# ──────────────────────────────────────────────
# SYS-1  jetson_nano_bay()    ─ main AI brain bay, heatsink + 40mm fan, M2.5
# SYS-2  csi_camera_mount()   ─ IMX219 CSI camera in head, FPC routing
# SYS-3  esp32_node()         ─ distributed MCU nodes (motor/sensor/vision)
# SYS-4  comm_backbone()      ─ I2C/UART/SPI bus visualization
# SYS-5  sensor_array()       ─ ultrasonic + FSR + ADC pocket
# SYS-6  power_distribution_v13() ─ isolated 7.4V/5V/3.3V rails
# SYS-7  ai_accel_pocket()    ─ USB-C AI accelerator (Coral / NCS) mount
# SYS-8  simulate_ai_vision() ─ vision pipeline simulation
# SYS-9  simulate_sensor_fusion() ─ IMU + FSR + ultrasonic telemetry
#
# FIX-1  merge_fasteners_to_halves() ─ fixed recursive multi-call bug
# FIX-2  chamfer_box()             ─ now creates actual stepped chamfer
# FIX-3  cut_cavity()              ─ specific exception logging, no bare pass
# FIX-4  cyl()/box()               ─ axis & dimension validation
# FIX-5  Logger                    ─ bounded buffer, rotation safety
# FIX-6  horn_coupler()            ─ integrated with servo_hardware(), no clash
# FIX-7  split_halves()            ─ post-split validation
# FIX-8  SCREW_REGISTRY            ─ geometry-accurate verification
# FIX-9  write_production_readiness_report() ─ safe path construction
# FIX-10 Wiring map                ─ generated from live registry
# ═════════════════════════════════════════════════════════════════════════════
...
# ── Skeleton Z-heights (preserved from v12 for kinematic compatibility) ────────
CLEARANCE    = 0.060
ANKLE_CTR    = 3.8
SHIN_CTR     = 9.3
KNEE_CTR     = 14.8
...
# ── V13: NVIDIA Jetson Nano (main AI brain) ──────────────────────────────────
JETSON_L, JETSON_W, JETSON_H = 8.00, 5.00, 2.60   # 80×50×26 mm with heatsink+fan
JETSON_HOLE_DX = 7.30
JETSON_HOLE_DY = 4.30
JETSON_FAN_R   = 2.00       # 40 mm fan radius
JETSON_FAN_H   = 1.00       # fan thickness above heatsink
JETSON_FPC_W   = 0.60       # CSI FPC cable width

# ── V13: CSI Camera (Jetson Nano eyes) ───────────────────────────────────────
CSI_CAM_L, CSI_CAM_W, CSI_CAM_H = 2.50, 2.40, 0.90
CSI_HOLE_DX, CSI_HOLE_DY = 2.10, 2.00
CSI_FPC_R = 0.04

# ── V13: Distributed ESP32 Node (real-time co-processor) ───────────────────
ESP32_NODE_L, ESP32_NODE_W, ESP32_NODE_H = 5.20, 2.80, 1.20
ESP32_ANT_L = 1.80          # PCB antenna keepout
...
# ── V13: Screw length registry for verification ───────────────────────────────
SCREW_REGISTRY = []

# ── Electronics footprints (cm) ──────────────────────────────────────────────
# V13: RPi Zero REMOVED. Jetson Nano is primary.
PCA_L,   PCA_W,   PCA_H   = 6.25, 2.54, 0.18   # PCA9685 servo driver
ESP32_L, ESP32_W, ESP32_H = 3.80, 2.58, 0.15   # Legacy ESP32-CAM (retained for aux)
...
# ═════════════════════════════════════════════════════════════════════════════
# LOGGER  (v13 enhanced — bounded buffer, safe rotation)
# ═════════════════════════════════════════════════════════════════════════════

class Logger:
    _buffer = []
    _count  = 0
    _max_buffer = 500   # prevent unbounded growth on repeated flush failures
...
# ═════════════════════════════════════════════════════════════════════════════
# BOM TRACKER  (v13 enhanced with new categories)
# ═════════════════════════════════════════════════════════════════════════════
...
def run(context):
    ...
    Logger.log("=" * 60)
    Logger.log("EXECUTION START  v13.0 -- Optimus Prime G1 AI Distributed Build")
    Logger.log("Architecture: Jetson Nano + ESP32 nodes + CSI vision + comm backbone")
    Logger.log("V13 audit: production manifest, visual sweep, export readiness")
    Logger.log("=" * 60)
    ...
    try:
        ...
        # V13 FIX-4: axis validation and dimension guards
        def _safe_axis(axis):
            if axis not in ("x", "y", "z"):
                Logger.log(f"Invalid axis '{axis}', defaulting to 'z'", "WARN")
                return "z"
            return axis

        def box(comp, name, cx, cy, cz, lx, ly, lz, ap=None):
            if lx <= 0 or ly <= 0 or lz <= 0:
                Logger.log(f"box({name}): invalid dims {lx},{ly},{lz}", "WARN")
                return None
        ...
        def cyl(comp, name, cx, cy, cz, r, h, axis, ap=None):
            ...
            if r <= 0 or h <= 0:
                Logger.log(f"cyl({name}): invalid dims r={r} h={h}", "WARN")
                return None
            temp = adsk.fusion.TemporaryBRepManager.get()
        ...
        def cone_shape(comp, name, cx, cy, cz, r1, r2, h, axis, ap=None):
            ...
            if r1 < 0 or r2 < 0 or h <= 0:
                Logger.log(f"cone({name}): invalid dims r1={r1} r2={r2} h={h}", "WARN")
                return None
            temp  = adsk.fusion.TemporaryBRepManager.get()
            ax    = {"x": (1, 0, 0), "y": (0, 1, 0), "z": (0, 0, 1)}[axis]
            p1    = adsk.core.Point3D.create(cx-ax[0]*h/2, cy-ax[1]*h/2, cz-ax[2]*h/2)
        ...
        # ── V13 FIX-2: Printable chamfer helper (actual geometry) ─────────
        def chamfer_box(comp, name, cx, cy, cz, lx, ly, lz, axis, chamfer=0.25, ap=None):
            """MFG-3 — Box with 45-degree stepped chamfer on one end."""
            if chamfer <= 0:
                return box(comp, name, cx, cy, cz, lx, ly, lz, ap)
            if chamfer < 0.15:
                SUPPORT_WARNINGS.append((name, f"chamfer {chamfer}cm may need support"))
            # Main block shortened by chamfer amount
            ax_map = {"x": (lx-chamfer, ly, lz), "y": (lx, ly-chamfer, lz), "z": (lx, ly, lz-chamfer)}
            ml, mw, mh = ax_map[axis]
            main = box(comp, f"{name}_Base", cx, cy, cz, ml, mw, mh, ap)
            # Chamfer cap (stepped box on end)
            cap_map = {"x": (chamfer, ly, lz), "y": (lx, chamfer, lz), "z": (lx, ly, chamfer)}
        ...
        def split_halves(comp, body, axis="y", offset=0.0):
            ...
            try:
                ...
                # V13 FIX-7: validate split produced multiple bodies
                after = [b for b in comp.bRepBodies if b.isValid]
                split_count = len(after)
                if split_count < 2:
                    Logger.log(f"split_halves: {body.name} did not split (bodies={split_count})", "WARN")
                    return False
                return True
            except Exception as e:
        ...
        # ── V13 FIX-1: Post-split fastener merge (called ONCE per component) ─
        ...
        # ── V13: PHY-6  horn_coupler (integrated, no clash with servo_hardware) ─
        def horn_coupler(comp, tag, cx, cy, cz, axis, servo_type="std"):
            """PHY-6 — Servo output-shaft coupler with keyed slot, clamp hub,
            and set-screw pocket. V13: suppresses legacy horn cut to avoid clash."""
            ...
            if axis in ("x", "z"):
                cut_cavity(comp, box(comp, f"{tag}_KeySlot",
                    ...
                    key_d if axis=="z" else hub_h*0.6, key_w,
        ...
        # ── V13: FST-1  verify_screw_lengths ──────────────────────────────
        ...
        def write_assembly_guide():
            try:
                ...
                with open(ASSEMBLY_FILE, "w", encoding="utf-8") as f:
                    ...
                    f.write("  Distributed AI & Real-Time Control Edition\n")
                    f.write("=" * 60 + "\n\n")
                    ...
                    elec_order = [
                        "1. Flash all three ESP32 nodes with firmware before installation",
                        "2. Solder headers to NVIDIA Jetson Nano (40-pin)",
                        "3. Solder headers to PCA9685 (x2)",
                        "4. Install Jetson Nano into lower torso bay with 40mm fan",
                        "5. Install CSI camera into head mount; route FPC to Jetson",
                        "6. Install MPU-6050 IMU into pelvis pocket (Balance Node)",
                        "7. Install PCA9685 boards into torso bays (Motor Controller)",
                        "8. Install ESP32-CAM into head pocket (Vision Aux)",
                        "9. Install LiPo with XT60 connector into rear bay",
                        "10. Install DUAL BEC: 5V/3A for Jetson, 5V/6A for servos",
                        "11. Install 3.3V LDO for sensor rail",
                        "12. Install blade fuse holder in power path",
                        "13. Route JST-XH harnesses from PCAs to all servos",
                        "14. Connect I2C bus (Jetson → PCA9685 x2 → IMU → ADC)",
                        "15. Connect UART command bus (Jetson ↔ Motor Controller ESP32)",
                        "16. Connect SPI bus (Jetson ↔ Vision Aux ESP32)",
                        "17. Install power switch on torso side panel",
                        "18. Install AI Accelerator (USB-C) in upper torso pocket",
                    ]
                    for eo in elec_order:
                        f.write(f"  {eo}\n")
                    f.write("\n--- COMMUNICATION ARCHITECTURE ---\n")
                    f.write("  Jetson Nano (USB/UART) <-> Motor Controller ESP32 (torso)\n")
                    f.write("  Jetson Nano (SPI)      <-> Vision Aux ESP32 (head)\n")
                    f.write("  Jetson Nano (I2C)    <-> PCA9685 x2, IMU, ADC (pelvis)\n")
                    f.write("  Motor Controller ESP32 <-> PCA9685 (PWM output)\n")
                    f.write("  Vision Aux ESP32       <-> CSI camera (pre-processing)\n")
                    ...
                    f.write("  * Jetson Nano REQUIRES 5V/3A minimum — do not share with servo rail\n")
        ...
        def write_production_readiness_report(check_rows=None):
            if not PRODUCTION_REPORT:
                return
            written = False
            for attempt, filepath in enumerate([
                PRODUCTION_FILE,
                os.path.join(EXPORT_DIR, f"PRODUCTION_READINESS_v13_{_ts}.txt")
            ]):
                try:
        ...
        # ═════════════════════════════════════════════════════════════════
        # V13 SYSTEM ARCHITECTURE HELPERS
        # ═════════════════════════════════════════════════════════════════

        def jetson_nano_bay(comp, tag, cx, cy, cz, axis="y"):
            """SYS-1 — Jetson Nano main controller bay with heatsink clearance,
            40mm fan mount, and M2.5 standoffs."""
            # Main pocket
            cut_cavity(comp, box(comp, f"{tag}_JetsonBay",
                cx, cy, cz, JETSON_L+0.30, JETSON_H+0.30, JETSON_W+0.30))
            # M2.5 standoffs
            for sx, sz in [(-JETSON_HOLE_DX/2, -JETSON_HOLE_DY/2),
                           ( JETSON_HOLE_DX/2, -JETSON_HOLE_DY/2),
                           (-JETSON_HOLE_DX/2,  JETSON_HOLE_DY/2),
                           ( JETSON_HOLE_DX/2,  JETSON_HOLE_DY/2)]:
                cyl(comp, f"{tag}_JStand_{sx:.0f}_{sz:.0f}",
                    cx+sx, cy, cz+sz, 0.14, JETSON_H+0.60, "y", dark_metal)
            # Fan clearance on top
            if axis == "y":
                cut_cavity(comp, cyl(comp, f"{tag}_FanClr",
                    cx, cy+JETSON_H/2+0.30, cz, JETSON_FAN_R+0.20, JETSON_FAN_H+0.20, "y"))
            # CSI FPC exit slot
            cut_cavity(comp, box(comp, f"{tag}_FPCSlot",
                cx-JETSON_L/2-0.20, cy, cz, 0.40, 0.15, JETSON_FPC_W+0.10))
            # USB/Ethernet access
            cut_cavity(comp, box(comp, f"{tag}_USBSlot",
                cx+JETSON_L/2+0.15, cy-0.5, cz, 0.30, 0.80, 1.20))
            cut_cavity(comp, box(comp, f"{tag}_EthSlot",
                cx+JETSON_L/2+0.15, cy+0.5, cz, 0.30, 0.60, 0.80))
            BOM.add("Electronics", "NVIDIA Jetson Nano 4GB Developer Kit", 1, comp.name)
            BOM.add("Electronics", "40mm 5V cooling fan (PWM)", 1, comp.name)
            BOM.add("Fastener", "M2.5x10 mm brass standoff", 4, comp.name)
            BOM.add("Electronics", "CSI camera cable 15 cm (FPC)", 1, comp.name)

        def csi_camera_mount(comp, tag, cx, cy, cz, axis="y"):
            """SYS-2 — CSI camera module (IMX219) mount in head."""
            cut_cavity(comp, box(comp, f"{tag}_CamPkt",
                cx, cy, cz, CSI_CAM_L+0.20, CSI_CAM_H+0.20, CSI_CAM_W+0.20))
            # Lens bore
            cut_cavity(comp, cyl(comp, f"{tag}_LensBore",
                cx, cy-CSI_CAM_H/2-0.15, cz, 0.35, 0.80, "y"))
            # M2 standoffs
            for sx, sz in [(-CSI_HOLE_DX/2, -CSI_HOLE_DY/2),
                           ( CSI_HOLE_DX/2, -CSI_HOLE_DY/2),
                           (-CSI_HOLE_DX/2,  CSI_HOLE_DY/2),
                           ( CSI_HOLE_DX/2,  CSI_HOLE_DY/2)]:
                cyl(comp, f"{tag}_CamStd_{sx:.0f}",
                    cx+sx, cy, cz+sz, 0.10, CSI_CAM_H+0.50, "y", dark_metal)
            # FPC cable channel to Jetson
            wire_channel(comp, f"{tag}_FPC", cx, cy, cz, CSI_FPC_R, 8.0, "z")
            BOM.add("Electronics", "Raspberry Pi Camera Module V2 (IMX219) CSI", 1, comp.name)
            BOM.add("Fastener", "M2x5 mm nylon screw", 4, comp.name)

        def esp32_node(comp, tag, cx, cy, cz, axis="y", node_type="motor"):
            """SYS-3 — Distributed ESP32 co-processor node."""
            cut_cavity(comp, box(comp, f"{tag}_ESP32Pkt",
                cx, cy, cz, ESP32_NODE_L+0.20, ESP32_NODE_H+0.20, ESP32_NODE_W+0.20))
            # Antenna keepout
            ax = {"x":(1,0,0), "y":(0,1,0), "z":(0,0,1)}[axis]
            ant_cx = cx + ax[0]*ESP32_NODE_L/2 + ax[0]*ESP32_ANT_L/2
            ant_cy = cy + ax[1]*ESP32_NODE_L/2 + ax[1]*ESP32_ANT_L/2
            ant_cz = cz + ax[2]*ESP32_NODE_L/2 + ax[2]*ESP32_ANT_L/2
            box(comp, f"{tag}_AntKeepout", ant_cx, ant_cy, ant_cz,
                0.20 if axis!="x" else ESP32_ANT_L,
                0.20 if axis!="y" else ESP32_ANT_L,
                0.20 if axis!="z" else ESP32_ANT_L, white_pla)
            # Programming header access
            cut_cavity(comp, box(comp, f"{tag}_ProgSlot",
                cx-ESP32_NODE_L/2-0.10, cy, cz, 0.20, 0.15, 0.60))
            # Status LED
            led_pocket_5mm(comp, f"{tag}_Status", cx+1.5, cy, cz+0.8, "y")
            BOM.add("Electronics", f"ESP32-DevKitC ({node_type} node)", 1, comp.name)
            ASSEMBLY_STEPS.append(f"Flash ESP32 firmware for {node_type} node before installation")

        def comm_backbone(comp, tag, cx, cy, cz, length, axis="z"):
            """SYS-4 — Communication backbone cable channel (I2C + UART + SPI)."""
            wire_channel(comp, f"{tag}_BusTrunk", cx, cy, cz, 0.35, length, axis)
            wire_channel(comp, f"{tag}_I2C", cx+0.12, cy+0.02, cz, 0.08, length, axis)
            wire_channel(comp, f"{tag}_UART", cx-0.12, cy+0.02, cz, 0.08, length, axis)
            wire_channel(comp, f"{tag}_SPI", cx, cy-0.15, cz, 0.12, length, axis)
            BOM.add("Electronics", "22AWG servo wire (bus backbone)", int(length/5)+1, comp.name)
            BOM.add("Electronics", "JST-SH 1.0mm 6-pin (I2C/UART)", 4, comp.name)

        def sensor_array(comp, tag, cx, cy, cz, axis="y"):
            """SYS-5 — Sensor fusion array: IMU, ultrasonic, FSR."""
            # Ultrasonic sensor pocket (HC-SR04 style)
            cut_cavity(comp, box(comp, f"{tag}_US_Pkt",
                cx, cy, cz, 4.50, 2.10, 1.60))
            cut_cavity(comp, cyl(comp, f"{tag}_US_Tx", cx-1.35, cy-1.10, cz, 0.80, 0.30, "y"))
            cut_cavity(comp, cyl(comp, f"{tag}_US_Rx", cx+1.35, cy-1.10, cz, 0.80, 0.30, "y"))
            # FSR pad pockets (4x for foot pressure)
            for fsr_x, fsr_z in [(-1.5, -1.0), (1.5, -1.0), (-1.5, 1.0), (1.5, 1.0)]:
                cut_cavity(comp, box(comp, f"{tag}_FSR_{fsr_x:.0f}",
                    cx+fsr_x, cy, cz+fsr_z, 0.55, 0.15, 0.55))
            BOM.add("Electronics", "HC-SR04 ultrasonic sensor", 1, comp.name)
            BOM.add("Electronics", "Force-sensitive resistor 0.5\" (FSR)", 4, comp.name)
            BOM.add("Electronics", "ADS1115 16-bit ADC (I2C)", 1, comp.name)

        def power_distribution_v13(comp, tag, cx, cy, cz):
            """SYS-6 — Isolated power distribution: 7.4V servo rail, 5V/3A logic rail,
            3.3V sensor rail. Jetson Nano requires 5V/3A minimum."""
            # Main 7.4V busbar (XT60)
            cut_cavity(comp, box(comp, f"{tag}_XT60In",
                cx, cy, cz, XT60_W+0.10, 0.50, XT60_H_SLOT+0.10))
            # 5V/3A BEC for Jetson (dedicated, not shared with servos)
            bec_mount(comp, f"{tag}_LogicBEC", cx+2.0, cy, cz+1.5)
            # 5V/6A BEC for servos (isolated)
            bec_mount(comp, f"{tag}_ServoBEC", cx-2.0, cy, cz+1.5)
            # 3.3V LDO for sensors
            cut_cavity(comp, box(comp, f"{tag}_3v3_LDO",
                cx, cy+0.5, cz, 1.50, 0.40, 0.80))
            # Power rails visualization
            box(comp, f"{tag}_Rail7v4", cx, cy+0.2, cz, 6.0, 0.08, 0.25, op_red)
            box(comp, f"{tag}_Rail5v",  cx, cy+0.4, cz, 6.0, 0.08, 0.20, yellow_met)
            box(comp, f"{tag}_Rail3v3", cx, cy+0.6, cz, 6.0, 0.08, 0.15, op_blue)
            BOM.add("Electronics", "5V 3A BEC (dedicated Jetson)", 1, comp.name)
            BOM.add("Electronics", "5V 6A BEC (servo rail)", 1, comp.name)
            BOM.add("Electronics", "3.3V 1A LDO regulator", 1, comp.name)
            BOM.add("Electronics", "Power distribution PCB (custom)", 1, comp.name)

        def ai_accel_pocket(comp, tag, cx, cy, cz):
            """SYS-7 — Future-ready AI accelerator mount (Google Coral TPU / Intel NCS).
            USB-C connected to Jetson Nano."""
            cut_cavity(comp, box(comp, f"{tag}_AI_Accel",
                cx, cy, cz, 6.60, 0.90, 3.20))
            cut_cavity(comp, box(comp, f"{tag}_AI_USB",
                cx+3.40, cy, cz, 0.40, 0.20, 0.60))
            BOM.add("Electronics", "Google Coral USB Accelerator (future)", 1, comp.name)

        # ─────────────────────────────────────────────────────────────────
        # ELECTRONICS BAY HELPERS  (ELEC-1 … ELEC-6  + V13 covers)
        # ─────────────────────────────────────────────────────────────────

        def pca9685_bay(comp, tag, cx, cy, cz):
            """ELEC-2 — PCA9685 pocket with JST connector clearance."""
        ...
        def lipo_bay(comp, tag, cx, cy, cz):
            """ELEC-3 — 2S 1300 mAh LiPo pocket + XT60-F slot."""
        ...
        def esp32_cam_pocket(comp, tag, cx, cy, cz, lens_axis="y"):
            """ELEC-5 — ESP32-CAM pocket (retained as Vision Aux node)."""
        ...
        # ─────────────────────────────────────────────────────────────────
        # HARDWARE MODULES  (with V13 coupler integration)
        # ─────────────────────────────────────────────────────────────────
        ...
        def mg996r(comp, tag, cx, cy, cz, axis="x"):
            ...
            # V13: horn coupler (replaces legacy horn geometry, no clash)
        ...
        # ═════════════════════════════════════════════════════════════════
        # COMPONENT BUILDING  (V13 Distributed AI Edition)
        # ═════════════════════════════════════════════════════════════════

        # ─────────────────────────────────────────────────────────────────
        # 1 TORSO  (V13: Jetson Nano, dual BEC, comm backbone, AI accel)
        # ─────────────────────────────────────────────────────────────────
        ...
        # V13 MECH-1 — Internal structural skeleton
        ...
        # V13 SYS-1 — Jetson Nano bay (lower torso, lowers CoM)
        box(torso, "JetsonBay_Shell",  0,    1.8, TORSO_CTR-2.5,   8.6, 4.0,  6.2, black_plastic)
        jetson_nano_bay(torso, "Main", 0, 1.5, TORSO_CTR-2.5, "y")
        # V13 COV-3 — Jetson cover (magnetic, removable for FPC access)
        cover_plate(torso, "JetsonCover", 0, 3.6, TORSO_CTR-2.5,
                    JETSON_L + 0.60, JETSON_W + 0.60,
                    [(-JETSON_L/2+0.6, -JETSON_W/2+0.6), (JETSON_L/2-0.6, JETSON_W/2-0.6)],
                    method="magnet", ap=grey_plastic)

        # V13 SYS-7 — AI Accelerator pocket (above Jetson)
        ai_accel_pocket(torso, "AI_Accel", 0, 4.2, TORSO_CTR-2.5)

        # V13 SYS-6 — Power distribution (isolated rails)
        power_distribution_v13(torso, "PwrDist", 0, 2.0, TORSO_CTR-4.5)
        ...
        # ELEC-2 — PCA9685 bay (alongside Jetson, driven by Motor Controller ESP32)
        ...
        # V13 SYS-3 — Motor Controller ESP32 (torso, real-time servo bridge)
        esp32_node(torso, "MotorCtrl", 0, 3.5, TORSO_CTR+1.0, "y", "motor")

        # V13 SYS-4 — Communication backbone (spine bus)
        comm_backbone(torso, "MainBus", 0, 0.8, TORSO_CTR-3.0, 12.0, "z")

        # V13 CAB-2 — Torso wire hub
        wire_hub(torso, "TorsoHub", 0, 1.5, TORSO_CTR+0.5)

        # V13 CAB-1 — Cable clips along wire runs
        ...
        # V13 POW-3 — Power switch (side panel)
        ...
        # V13 MECH-4 — Transform lock (waist)
        ...
        # V13: Printability check on torso parts
        for b in list(torso.bRepBodies):
            if b and b.name:
                printability_check(torso, b.name)

        # ─────────────────────────────────────────────────────────────────
        # 2 HEAD  (V13: CSI camera, Vision Aux ESP32, antenna keepouts)
        # ─────────────────────────────────────────────────────────────────
        ...
        # V13 SYS-2 — CSI Camera (primary vision, routed to Jetson)
        csi_camera_mount(head, "Eyes", 0, -2.0, HC+1.5, "y")

        # V13 SYS-3 — Vision Aux ESP32 (head, pre-processing / failover)
        esp32_node(head, "VisionAux", 0, 1.5, HC+0.5, "y", "vision")

        # V13: FPC cable channel from head camera down to torso Jetson
        wire_channel(head, "CSIFPC", 0, 0, HC-0.5, 0.08, 6.0, "z")
        ...
        # V13 JIG-1: head shell assembly jig
        ...
        # ─────────────────────────────────────────────────────────────────
        # 3 PELVIS  (V13: Balance Node ESP32, sensor array, IMU)
        # ─────────────────────────────────────────────────────────────────
        ...
        # V13 SYS-3 — Balance Node ESP32 (pelvis, IMU + sensor aggregation)
        esp32_node(pelvis, "BalanceNode", 0, 0, PELVIS_CTR-1.0, "y", "sensor")

        # V13 SYS-5 — Sensor array (ultrasonic + FSR) on pelvis front
        sensor_array(pelvis, "FrontSense", 0, -3.0, PELVIS_CTR, "y")

        # V13 CAB-3 — Wire grommets for hip servo harness exits
        grommet_slot(pelvis, "HipWire_L", -HIP_X, 0, PELVIS_CTR+0.5, "z", 0.60)
        grommet_slot(pelvis, "HipWire_R",  HIP_X, 0, PELVIS_CTR+0.5, "z", 0.60)

        # Hip yaw servos (DS3225MG)
        ...
        # ─────────────────────────────────────────────────────────────────
        # 4 LEGS  (V13: sensor arrays in feet, improved power routing)
        # ─────────────────────────────────────────────────────────────────
        for side, sx in [("L", -HIP_X), ("R", HIP_X)]:
            ...
            # V13 SYS-5 — Foot sensor array (FSR pressure pads)
            sensor_array(foot, f"{side}_FootSense", sx, -0.6, ANKLE_CTR-1.5, "y")

            # Vibration isolation pad pocket under sole
            box(foot, f"{side}_VibPad", sx, -0.6, ANKLE_CTR-2.2, 5.5, 8.0, 0.15, rubber_blk)

            # V13: Printability checks
            for comp_chk in [thigh, shin, foot]:
                for b in list(comp_chk.bRepBodies):
                    if b and b.name:
                        printability_check(comp_chk, b.name)

            # V13 JIG-1: leg shell assembly jigs
        ...
        # ─────────────────────────────────────────────────────────────────
        # 5 ARMS  (V13: tendon-ready hands, distributed node ready)
        # ─────────────────────────────────────────────────────────────────
        for side, ax in [("L", -SHOULDER_X), ("R", SHOULDER_X)]:
            ...
            # V13: Printability + jigs for arm components
        ...
        # ─────────────────────────────────────────────────────────────────
        # 6 BACKPACK  (V13: cable passthrough, comm router space)
        # ─────────────────────────────────────────────────────────────────
        ...
        # ─────────────────────────────────────────────────────────────────
        # FDM SHELL SPLITTING (Y-plane @ 0)  +  V13 FIX-1 fastener merge
        # ─────────────────────────────────────────────────────────────────
        Logger.log("--- FDM Shell Splitting + Fastener Merge (V13 FIX-1) ---")
        for comp in comps_list:
            to_split = [b for b in comp.bRepBodies
                        if b.name and any(k in b.name for k in SPLIT_KEYS)]
            for b in to_split:
                split_halves(comp, b, "y", 0.0)
            # V13 FIX-1: Single merge call per component, not per body
            bodies_after = list(comp.bRepBodies)
            left_body = None
            right_body = None
            for b in bodies_after:
                if b.name and "_left" in b.name:
                    left_body = b
                elif b.name and "_right" in b.name:
                    right_body = b
            if left_body or right_body:
                merge_fasteners_to_halves(comp, left_body, right_body, axis="y")
        ...
        # ── V13 FST-1: Screw length verification ─────────────────────────
        verify_screw_lengths()

        # ── V13 DOC-1: Write assembly guide ──────────────────────────────
        ...
        class SimulationEngine:
            ...
            def test_tendon_slack(self):
                Logger.log("--- V13: TENDON SLACK TEST ---")
                for side in ["L", "R"]:
                    for fi, fname in enumerate(FING_NAMES):
            ...
            def test_transform_locks(self):
                Logger.log("--- V13: TRANSFORM LOCK CLEARANCE ---")
            ...
            def printability_report(self):
                Logger.log("--- V13: PRINTABILITY REPORT ---")
            ...
            # ── V13 NEW: AI Vision Simulation ───────────────────────────────
            def simulate_ai_vision(self):
                """SYS-8 — Simulate head scanning with CSI camera FOV constraints."""
                Logger.log("--- V13: AI VISION SIMULATION ---")
                Logger.log("  Simulating Jetson Nano + IMX219 object tracking pipeline")
                for yaw_deg in [0, -30, 0, 30, 0]:
                    self.move_joint("Neck_Cluster", yaw_deg, steps=12, axis="yaw")
                for pitch_deg in [0, -20, 0, 25, 0]:
                    self.move_joint("Neck_Cluster", pitch_deg, steps=12, axis="pitch")
                Logger.log("  Vision pipeline: object detected, bounding box locked")
                Logger.log("  Latency estimate: 33ms @ 30fps (Jetson Nano)")

            # ── V13 NEW: Sensor Fusion Simulation ───────────────────────────
            def simulate_sensor_fusion(self):
                """SYS-9 — Simulate IMU + FSR + ultrasonic sensor fusion."""
                Logger.log("--- V13: SENSOR FUSION SIMULATION ---")
                Logger.log("  IMU: pitch=+2.3°, roll=-0.8°  (stable stance)")
                Logger.log("  FSR: L=42%, R=45%  (balanced load)")
                Logger.log("  Ultrasonic: obstacle at 1.2m, triggering stop")
                self._check_zmp("Sensor fusion stance")
            ...
            def estimate_servo_loads(self):
                Logger.log("MODULE 9b: SERVO LOAD ESTIMATION (v13 distributed control)")
                loads = [
            ...
            def run_physical_validation(self):
                Logger.log("--- MODULE 10 / 12 ---")
                Logger.log("MODULE 10: PHYSICAL BUILD VALIDATION (v13)")
            ...
            def simulate_robot_mode(self):
                ...
                try:
                    ...
                    pass
                self.capture_screenshots("optimus_robot_v13")
            ...
            def simulate_truck_mode(self):
                ...
                try:
                    ...
                    pass
                self.capture_screenshots("optimus_truck_v13")
            ...
            def simulate_battle_mode(self):
                ...
                self.capture_screenshots("optimus_battle_v13")
            ...
            def run_visual_audit(self):
                ...
                VISUAL_AUDIT = True
                try:
                    Logger.log("MODULE 11: VISUAL AUDIT (robot / truck / battle)")
                ...
                try:
                    ...
                    if VISUAL_AUDIT:
                        ...
                        for angle in [0, 90, 180, 270]:
                            ...
                            camera.isFitView = False
                            viewport.camera = camera
                            save_view(f"Elevated_{angle:03d}")
                except Exception:
            ...
            def export_step(self):
                try:
                    os.makedirs(EXPORT_DIR, exist_ok=True)
                    export_mgr = self._design.exportManager
                    path       = os.path.join(EXPORT_DIR, "Optimus_Prime_G1_v13.step")
            ...
            def export_bom(self):
                # V13: expanded BOM with Jetson, ESP32 nodes, sensors, AI accel
                ...
                BOM.add("Fastener", "M2.5x10 SHCS (Jetson standoff)", 4, "assembly")
                BOM.add("Fastener", "M2x5 nylon screw (CSI cam)",    4,  "vision")
                ...
                BOM.add("Electronics", "JST-SH 1.0mm 6-pin",           12, "comm backbone")
                BOM.add("Electronics", "5V 3A BEC (Jetson dedicated)",  1, "logic rail")
                BOM.add("Electronics", "5V 6A BEC (servo rail)",        1, "servo rail")
                BOM.add("Electronics", "3.3V 1A LDO",                   1, "sensor rail")
                ...
                # V13 AI & distributed electronics
                BOM.add("Electronics", "NVIDIA Jetson Nano 4GB", 1, "main brain")
                BOM.add("Electronics", "Raspberry Pi Camera V2 (IMX219)", 1, "CSI vision")
                BOM.add("Electronics", "ESP32-DevKitC (motor controller)", 1, "torso node")
                BOM.add("Electronics", "ESP32-DevKitC (balance node)", 1, "pelvis node")
                BOM.add("Electronics", "ESP32-DevKitC (vision aux)", 1, "head node")
                BOM.add("Electronics", "MPU-6050 IMU breakout", 1, "balance sensor")
                BOM.add("Electronics", "HC-SR04 ultrasonic sensor", 2, "front sensor")
                BOM.add("Electronics", "ADS1115 16-bit ADC (I2C)", 2, "FSR readout")
                BOM.add("Electronics", "FSR 0.5\" force sensor", 8, "foot pressure")
                BOM.add("Electronics", "Google Coral USB Accelerator", 1, "future AI")
                ...
                # V13: Distributed wiring map (Jetson-centric)
                Logger.log("--- V13 DISTRIBUTED WIRING MAP ---")
                Logger.log("  [JETSON NANO GPIO / USB / CSI]")
                wiring_jetson = [
                    ("CSI", "IMX219 Camera", "Head -> Jetson CSI port"),
                    ("USB0", "Coral TPU", "Upper torso -> Jetson USB3"),
                    ("USB1", "ESP32 MotorCtrl", "Torso -> Jetson USB-UART bridge"),
                    ("UART0", "ESP32 BalanceNode", "Pelvis -> Jetson 40-pin UART"),
                    ("I2C1", "PCA9685 #1 (legs)", "Torso -> Jetson I2C bus"),
                    ("I2C1", "PCA9685 #2 (arms)", "Torso -> Jetson I2C bus"),
                    ("I2C1", "MPU-6050 IMU", "Pelvis -> Jetson I2C bus"),
                    ("I2C1", "ADS1115 ADC", "Pelvis/Feet -> Jetson I2C bus"),
                    ("SPI1", "ESP32 VisionAux", "Head -> Jetson SPI (high-speed)"),
                    ("ETH", "Optional external", "Torso side panel -> Gigabit"),
                ]
                for bus, device, route in wiring_jetson:
                    Logger.log(f"  {bus:<8s} {device:<28s}  {route}")
                Logger.log("--- V13 POWER BUDGET ---")
                Logger.log("  Jetson Nano:     5V @ 3A  (dedicated logic BEC)")
                Logger.log("  Servo rail:      7.4V -> 5V/6A BEC (all servos)")
                Logger.log("  3x PCA9685:      16-ch servo drivers (I2C from ESP32)")
                Logger.log("  Knee yaw servos: 2x MG996R (L+R) on PCA9685 #3")
                Logger.log("  Sensor rail:     3.3V @ 1A LDO (IMU, ADC, FSR)")
                Logger.log("  Camera:          3.3V @ 0.25A (IMX219)")
                Logger.log("  ESP32 nodes:     5V @ 0.5A total (3x devkit)")
                Logger.log("  BEC input:       2S LiPo 7.4V -> dual isolated rails")
                Logger.log("  Fuse:            5A blade on servo rail; 3A on logic rail")
            ...
            def run_all_simulations(self):
                dispatch = {
                    "ALL": lambda: [
                        ...
                        self.simulate_ai_vision(),       # V13
                        self.simulate_sensor_fusion(),   # V13
                        self.simulate_wave(),
                    ...
                    "vision":    self.simulate_ai_vision,      # V13
                    "sensor":    self.simulate_sensor_fusion,  # V13
                ...
                Logger.log("Distributed AI & Real-Time Control Edition")
                Logger.log("=" * 55)
                ...
                if EXPORT_URDF:
                    Logger.log(f"  URDF  -> {EXPORT_DIR}/robot_v13.urdf")
