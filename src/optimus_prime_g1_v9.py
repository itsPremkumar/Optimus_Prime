...
#  ──────────────────────────────────────────────────────────────────────────
#  FIX 1  _make_joint_geometry()
#         SketchPoints on the XY construction plane silently discard the Z
#         coordinate, placing every joint origin at z = 0 instead of the
#         intended 3-D position.
#         → Now uses ConstructionPoint.createInput / setByPoint so all three
#           axes (x, y, z) are preserved correctly.
#
#  FIX 2  bearing()  — off-centre cavity
#         The cut-cylinder was built with p1 = (cx, cy, cz) and
#         p2 = (cx + Δ*(w+0.1), …), so the cavity started at the bearing
#         centre and extended entirely to one side.
#         → Now symmetric: p1 = centre − half, p2 = centre + half, where
#           half = w/2 + 0.05 cm clearance.
#
#  FIX 3  cut_cavity()  — mutable-collection iteration
#         Iterating comp.bRepBodies while combineFeatures.add() can implicitly
#         add or remove bodies causes skipped bodies and undefined behaviour.
#         → Snapshots the collection first with list(comp.bRepBodies).
#
#  FIX 4  reset_all()  — groups parameter silently ignored
#         The groups argument was computed but never used; ball_targets and
#         rev_targets always reset every joint regardless of the caller's intent.
#         → Now filters BALL_JOINTS / REV_JOINTS by the supplied groups list.
#
#  FIX 5  move_ball()  — revolute joints in mixed target lists
#         Walking and running animations passed L_Knee / R_Knee (revolute joints)
#         to move_ball().  RevoluteJointMotion only exposes rotationValue; it has
#         no pitchValue / yawValue / rollValue, causing silent failures.
#         → Now detects RevoluteJointMotion via objectType and routes the pitch
#           component to rotationValue; yaw and roll are skipped for revolute joints.
#
#  FIX 6  export_stl()  — non-existent body.exportSTL() method
#         BRepBody has no exportSTL member in the Fusion 360 Python API.
#         → Replaced with design.exportManager.createSTLExportOptions(body, path)
#           followed by exportManager.execute(options), which is the correct API.
#
#  FIX 7  doc.saveAs()  — wrong argument signature
#         saveAs(name, path_string) is not the Fusion 360 API; the overload
#         expects (name, DataFolder, description, tag) for cloud documents.
#         → Replaced with exportManager.createFusionArchiveExportOptions(path)
#           to write a local .f3d archive, which is the correct local-save API.
#
#  FIX 8  run_stability_analysis()  — non-existent measurePhysicalProperties
#         measureManager.measurePhysicalProperties(product) does not exist.
#         → Replaced with design.physicalProperties (returns a PhysicalProperties
#           object whose .centerOfMass gives the design-wide CoM).
#
#  FIX 9  _interfere()  — non-existent measureInterference API
#         measureManager.measureInterference() / design.measureInterference()
#         do not exist in the Python API.
#         → Primary path now uses the correct InterferenceInput approach:
#           design.createInterferenceInput(bodies) → design.interferenceAnalysis().
#           The existing fallback try/except chain is retained for API-version
#           compatibility.
# ═════════════════════════════════════════════════════════════════════════════
...
# ROM limits — used by test_joint_rom() and transformation validation.
...
def run(context):
    ...
    try:
        ...
        # FIX 2: The bearing cavity cylinder was asymmetric — p1 sat exactly
        #         at the bearing centre and p2 was offset by +(w+0.1), meaning
        #         the cavity extended entirely to one side.  Now both endpoints
        ...
        def bearing(comp, tag, cx, cy, cz, axis="x", ro=1.10, w=0.60):
            ...
            temp   = adsk.fusion.TemporaryBRepManager.get()
            ax     = {"x": (1, 0, 0), "y": (0, 1, 0), "z": (0, 0, 1)}[axis]
            half   = w / 2.0 + 0.05                                # ← FIX 2
            p1     = adsk.core.Point3D.create(cx - ax[0]*half,
                                              cy - ax[1]*half,
                                              cz - ax[2]*half)
            p2     = adsk.core.Point3D.create(cx + ax[0]*half,
                                              cy + ax[1]*half,
                                              cz + ax[2]*half)
        ...
        class SimulationEngine:
            ...
            def _gj(self, name):
                """Return the named AsBuiltJoint, or None with a log warning."""
                try:
            ...
            def _get(self, mo, axis):
                """Read current joint angle (radians) for the given axis."""
                try:
            ...
            def _set(self, mo, axis, val):
                """Write joint angle (radians) for the given axis."""
                try:
                    if mo.objectType == adsk.fusion.RevoluteJointMotion.classType():
            ...
            def _refresh(self):
                """Pump Fusion events; abort if the stop-flag file is present."""
            ...
            def _clamp(self, joint_name, axis, deg):
                """Clamp deg to the declared ROM limit; warn if out of range."""
            ...
            def move_joint(self, name, deg, steps=20, axis="pitch", ease=True, clamp=True):
                """Animate a single DOF on any joint type."""
                j = self._gj(name)
                if not j:
                    return
                if clamp:
            ...
            def move_group(self, targets, steps=20, axis="pitch", ease=True, clamp=True):
                """Animate multiple DOFs simultaneously (one axis each)."""
                active = []
                for item in targets:
                    name = item[0]
                    deg  = item[1]
            ...
            # FIX 5: move_ball() previously failed silently when revolute joints
            #         (e.g. L_Knee, R_Knee) appeared in the target list because
            #         RevoluteJointMotion exposes only rotationValue — it has no
            #         pitchValue / yawValue / rollValue attributes.
            #         The fix detects the joint type via objectType and:
            #           • For revolute joints  → routes the pitch component to
            #             rotationValue; yaw and roll are skipped (not applicable).
            #           • For ball joints      → routes each axis to the matching
            #             xyzValue property as before.
            def move_ball(self, targets, steps=20, clamp=True):
                """
                Animate ball-joint (pitch/yaw/roll) and revolute (pitch) targets
                in a single synchronised pass.  Pass None for axes to skip.

                targets: list of (name, pitch, yaw, roll)
                         For revolute joints only pitch is used; yaw/roll ignored.
                """
                active = []
                for name, pitch, yaw, roll in targets:
                    ...
                    is_rev   = (mo.objectType ==
                                adsk.fusion.RevoluteJointMotion.classType())  # ← FIX 5

                    if is_rev:
                        # Revolute joints only support a single rotation axis.
                        if pitch is not None:
                            v = self._clamp(name, "pitch", pitch) if clamp else pitch
                            active.append((mo, "pitch",
                                           self._get(mo, "pitch"), math.radians(v)))
                        # yaw and roll are meaningless for a revolute joint — skip.
                    else:
            ...
            # FIX 4: reset_all() used to compute `groups` but never apply it;
            #         ball_targets and rev_targets always reset every joint.
            #         Now the sets are filtered by `groups` when supplied.
            def reset_all(self, steps=10, groups=None):
                """
                Return all joints to 0°.  If `groups` is provided (list of joint
                names), only those joints are reset; others are left in place.
                """
                if groups is None:
                    ball_names = self.BALL_JOINTS          # ← FIX 4
                ...
                rev_targets  = [(n, 0.0)            for n in rev_names  if self._gj(n)]
            ...
            #         design.measureInterference(product),
            #         Design.cast(product).measureInterference(product).
            #         None of these exist in the Fusion 360 Python API.
            #
            #         The correct approach is to collect all bodies into an
            #         ObjectCollection, call design.createInterferenceInput(),
            #         then design.interferenceAnalysis().  The legacy fallback
            ...
            def _interfere(self, label="Interference"):
                try:
                    ...
                    for comp in self._comps:
                        for body in list(comp.bRepBodies):      # ← FIX 3 bonus
                            if body.name and not any(
                                    t in body.name for t in SKIP_TAGS):
                                all_bodies.add(body)
            ...
            def simulate_walking_advanced(self):
                ...
                for cycle in range(4):
                    ...
                    self.move_ball([
                        ...
                        ("L_Knee",             60,        None,       None),   # revolute
                        ("R_Knee",             60,        None,       None),   # revolute
            ...
            def simulate_running(self):
                ...
                for cycle in range(3):
                    ...
                    self.move_ball([
                        ...
                        ("L_Knee",             95,        None,      None),    # revolute
                        ("R_Knee",             95,        None,      None),    # revolute
            ...
            # Extracted to avoid duplicating the 5-stage sequence between
            # simulate_transformation() and simulate_truck_mode().
            def _transform_to_truck(self, steps_scale=1.0):
                """Apply all 5 transformation stages (Robot → Truck)."""
                s = steps_scale

                # Stage 1: Straighten elbows, fold blaster, fold wrists
                self.move_group([
                    ("R_Elbow",      0,   "pitch"),
                    ("L_Elbow",      0,   "pitch"),
                ...
                # Stage 2: Tuck head backward into chest cavity
                self.move_ball([("Neck_Cluster", -90, 0, 0)], steps=int(15*s))

                # Stage 3: Fold shoulders backward against torso
                self.move_ball([
                    ("L_Shoulder_Cluster", -88, 0, 0),
                    ("R_Shoulder_Cluster", -88, 0, 0),
                ], steps=int(22*s))

                # Stage 4: Hip yaw — backward fold 90°
                self.move_ball([
                    ("L_Hip_Cluster", 0, 90, 0),
                    ("R_Hip_Cluster", 0, 90, 0),
                ], steps=int(22*s))

                # Stage 5: Ankle fold flat
                self.move_ball([
            ...
            def _transform_to_robot(self, steps_scale=1.0):
                """Reverse all 5 transformation stages (Truck → Robot)."""
                s = steps_scale
                self.move_ball([
            ...
            # FIX 8: measureManager.measurePhysicalProperties() does not exist.
            ...
            # ── STL Export ────────────────────────────────────────────────
            # FIX 6: body.exportSTL() does not exist on BRepBody.
            #         Replaced with the correct API:
            ...
            def export_stl(self):
                try:
                    ...
                    skip       = {"Marker", "Pivot", "MtA", "MtB",
                                  "Axle_Pivot", "Horn", "_Vis", "Scope", "Antenna"}
                    export_mgr = self._design.exportManager   # ← FIX 6
                    count      = 0
                    ...
                    for comp in self._comps:
                        for body in list(comp.bRepBodies):    # ← FIX 3 bonus
                            ...
                            try:
                                path = os.path.join(
                                    EXPORT_DIR, f"{comp.name}_{body.name}.stl")
            ...
            def capture_screenshots(self, prefix="optimus"):
                ...
                try:
                    ...
                    Logger.log(
                        f"Screenshot capture failed: {traceback.format_exc()}", "WARN")
        ...
        # FIX 7: doc.saveAs(name, path_string) is not a valid Fusion 360 call.
        #         The overload expects (name, DataFolder, description, tag) for
        #         cloud saves.  For a local archive we use exportManager's
        #         createFusionArchiveExportOptions(filepath) instead, which
